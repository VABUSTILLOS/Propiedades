...
/**
 * Shared edit surface for the owner (and admins). Hydrates the same
 * single-form editor with the saved row; the advanced section and the
 * admin-chosen status are preserved on save. Server actions allow owners and
 * admins past the ownership check, matching the "Owners manage own properties"
 * and "Admins manage all properties" RLS policies.
 */
export default async function EditListingPage({
  ...
  const backHref = isAdmin ? "/admin/propiedades" : `/property/${property.slug}`;
  const backLabel = isAdmin ? "Volver al panel" : "Volver a la propiedad";
    ...
    <PageShell>
      ...
      <div className="mt-8">
        <ListingWizard
          ...
          adminPanelHref={isAdmin ? "/admin/propiedades" : undefined}
        />
      </div>
    </PageShell>
  );
...
function toWizardData(p: PropertiesRow) {
  return {
    title: p.title,
    type: p.type,
    category: p.category,
