#!/usr/bin/env node
/**
 * patch-benchmarks-41.mjs — upsert the 30 market_benchmarks for colonias that
 * first appeared in dataset-41 (2020–2022 root captures).
 *
 ...
 * land = avg for terreno (terreno_m2). Historical growth rate = 0.
 */
import fs from 'node:fs';
process.loadEnvFile('.env.local');
const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
const curated = JSON.parse(fs.readFileSync('/tmp/benchmark-curated-41.json', 'utf8'));
