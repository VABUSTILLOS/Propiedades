#!/usr/bin/env node
/**
 * extract-45.mjs — extract venta listings (≤ $3,000,000 MXN) from the
 ...
 *
 * Usage: node scripts/extract-45.mjs [--out file.jsonl] [--dir /tmp/captures-45]
 */
import fs from "node:fs";

const MAX_PRICE = 3_000_000;
const CAPTURE_DIR = process.argv.find((a) => a.startsWith("--dir="))?.split("=")[1] ?? "/tmp/captures-45";
const OUT =
  process.argv.find((a) => a.startsWith("--out="))?.split("=")[1] ??
  ".scrape/propiedades-com/dataset-45.jsonl";
...
function extractCaptureDate(f) {
  // date followed by optional -pN suffix (e.g. terrenos-industriales-venta-20191209-p2.html)
  let m = f.match(/-(\d{8})(?:-p\d+)?\.html$/);
  if (m) {
    const d = m[1];
...
function extractCategory(f) {
  // prefixed form: chihuahua-chihuahua-casas-venta-p2-20190428.html
  let m = f.match(/^chihuahua-chihuahua-(.+?)-p\d+-\d{8}\.html$/);
  if (m) return m[1];
  // prefixed form, date-before-page: chihuahua-chihuahua-casas-venta-20190428-p2.html
  m = f.match(/^chihuahua-chihuahua-(.+?)-\d{8}(?:-p\d+)?\.html$/);
  if (m) return m[1];
  // bare form: casas-venta-p2-20190428.html
  m = f.match(/^(.+?)-p\d+-\d{8}\.html$/);
  if (m) return m[1];
  // bare form, date-before-page: casas-venta-20190428-p2.html
  m = f.match(/^(.+?)-\d{8}(?:-p\d+)?\.html$/);
  if (m) return m[1];
  return null;
}
...
function extractSchema2b(html, captureDate, category) {
  ...
  for (const part of parts.slice(1)) {
    ...
    const typeM = part.match(/<span data-value="\d+">\s*([^<]+?)\s*<\/span>\s*(?:<br\s*\/?>)?\s*en\s*\n?Colonia/i);
    const typeSpan = typeM ? typeM[1].trim() : null;

    const pcM = part.match(/itemprop="postalCode">([^<]+)</);
    const postalCode = pcM ? pcM[1] : null;

    // Price: prefer the data-ga attribute ("<id>|V|<type>|<colonia>|<price>|..."),
    // then the footer-img price div, then the rendered price text.
    let price = null;
    const gaM = part.match(/data-ga="\d+\|[RV]\|[^|]*\|[^|]*\|(\d+)\|/);
    if (gaM) {
      price = Math.round(parseFloat(gaM[1]));
    }
    if (price === null) {
      const footM = part.match(/footer-img[\s\S]*?data-value="([\d.]+)"\s*data-value-min="[\d.]+"/);
      if (footM) {
        price = Math.round(parseFloat(footM[1]));
      }
    }
    if (price === null) {
      const priceM = part.match(/class="price-row">\s*<b>\s*([^<]+)<\/b>/);
...
for (const f of files) {
  ...
  if (html.includes('id="__NEXT_DATA__"')) {
    items = extractSchema4(html, captureDate, category);
    console.log(`${f}: ${items.length} venta ≤ $3M (schema-4)`);
  } else if (html.includes('class="properties-list"')) {
    items = extractSchema3(html, captureDate, category);
    console.log(`${f}: ${items.length} venta ≤ $3M (schema-3)`);
  } else if (html.includes('class="property-gallery')) {
...
/* ------------------------------------------------------------------ *
 *  Schema-4 (2024–2026): Next.js __NEXT_DATA__ structured JSON
 * ------------------------------------------------------------------ */

function extractSchema4(html, captureDate, category) {
  ...
  for (const p of props) {
    const price = p.sale_price_real ?? p.price_real;
    if (!price || price > MAX_PRICE) continue;
    if (p.purpose !== 1 && String(p.purpose_str).toLowerCase() !== "venta") continue;
    const id = String(p.id ?? "");
    if (!id) continue;
    const typeLabel = p.type_str || CATEGORY_TYPES[category] || "Propiedad";
    const neighborhood = p.colony || "";
    const datePosted = (p.published || captureDate || "").slice(0, 10);
    items.push({
      listing_id_propiedades: id,
      title: `Se vende ${String(typeLabel).toLowerCase()} en ${neighborhood || "Chihuahua"}, Chihuahua, ID: ${id}`,
      url: p.url_property || `https://propiedades.com/inmuebles/${id}`,
      price,
      price_currency: p.currency || "MXN",
      operation_type: "Venta",
      property_type: typeLabel,
      street_address: p.short_address || p.address1 || "",
      neighborhood,
      ...
      images: p.picture ? [p.picture] : [],
      main_image: p.picture || null,
      date_posted: datePosted,
      description: `${typeLabel} en venta en ${p.full_address || neighborhood || "Chihuahua"}, Chihuahua`,
      latitude: p.latitude && p.valid_coords ? parseFloat(p.latitude) : null,
      longitude: p.longitude && p.valid_coords ? parseFloat(p.longitude) : null,
      _capture: `${category}-${captureDate?.replace(/-/g, "")}`,
    });
  }
  return items;
