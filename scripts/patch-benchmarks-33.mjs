#!/usr/bin/env node
/**
 * Populate market_benchmarks for 28 newly-imported propiedades.com colonias
 * (dataset 33) that lacked benchmark coverage.
 *
 ...
 * avg_price_m2_land (avg_price_m2_const = 0); mixed colonias (Cordilleras,
 * Francisco R Almada, Granjas del Valle, Robinson) get both columns from the
 * respective sample sets. All others get avg_price_m2_const with
 * avg_price_m2_land = 0.
 *
 * Usage: node scripts/patch-benchmarks-33.mjs
 */
...
async function main() {
  ...
  const curated = JSON.parse(fs.readFileSync(path.join(root, 'scripts/benchmark-curated-33.json'), 'utf8'));
