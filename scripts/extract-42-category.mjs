#!/usr/bin/env node
/**
 * extract-42-category.mjs — extract venta listings (≤ $3,000,000 MXN) from the
 * unconsumed category captures of
 *   https://propiedades.com/chihuahua-chihuahua/{categoria}-venta
 *
 * Two schemas are handled:
 *   1. 2018–2021 era: server-rendered `.properties-list` cards (schema-3)
 *   2. 2015–2016 era: old S3 server-rendered `.listado-lista` cards (schema-2)
 *
 * Output: JSONL matching the import-propiedades.mjs contract.
 *
 * Usage: node scripts/extract-42-category.mjs [--out file.jsonl] [--dir /tmp/captures-42]
 */
import fs from "node:fs";

const MAX_PRICE = 3_000_000;
const CAPTURE_DIR = process.argv.find((a) => a.startsWith("--dir="))?.split("=")[1] ?? "/tmp/captures-42";
const OUT =
  process.argv.find((a) => a.startsWith("--out="))?.split("=")[1] ??
  ".scrape/propiedades-com/dataset-42-category.jsonl";
...
/** "123" or "123.45" from filename timestamp. */
function extractCaptureDate(f) {
  const m = f.match(/-(\d{8})(\d{6})?\.html$/);
  if (!m) return null;
  const d = m[1];
...
function extractCategory(f) {
  const m = f.match(/^(.+?)-venta-\d+\.html$/);
  if (!m) return null;
  const cat = m[1].replace(/-(\d+)$/, "");
  return cat;
}
