#!/usr/bin/env node
/**
 * patch-benchmarks-43.mjs — upsert the market_benchmarks for colonias that
 * first appeared in dataset-43 (2019 ?pagina=N captures).
 *
 ...
 * rate = 0.
 *
 * Run: node scripts/patch-benchmarks-43.mjs
 */
import fs from 'node:fs';
process.loadEnvFile('.env.local');
const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
const curated = JSON.parse(fs.readFileSync('/tmp/new-bench-43.json', 'utf8'));
const headers = {
  apikey: key,
